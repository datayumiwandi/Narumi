...
async fn tunnel(req: Request, mut cx: RouteContext<Config>) -> Result<Response> {
    ...
    if upgrade == "websocket".to_string() && PROXYIP_PATTERN.is_match(&proxyip) {
        ...
        Response::from_html("hi from yumi! check mayumiwandi in github")
    }
...
fn link(_: Request, cx: RouteContext<Config>) -> Result<Response> {
    ...
    let vless_link = format!("vless://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FKR&security=tls&sni={host}#shiroko vless");
    let trojan_link = format!("trojan://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FKR&security=tls&sni={host}#shiroko trojan");
    let ss_link = format!("ss://{}@{host}:443?plugin=v2ray-plugin%3Btls%3Bmux%3D0%3Bmode%3Dwebsocket%3Bpath%3D%2FKR%3Bhost%3D{host}#shiroko ss", URL_SAFE.encode(format!("none:{uuid}")));
